...
cat(capture.output(print(numberofavalibleDrugsCelllineList),
                   file = paste0(getwd(),"/output_SMOTE/numberofavalibleDrugsCelllineList.txt")))
...
for (i in 1:length(levels(as.factor(IC50CCLE$drug)))){
  ...
  pdf(paste0(getwd(),"/output_SMOTE/",drugg,"_GDSC.pdf"))  # The pdf receiving the output
  ...
  #======================================================================================================
  # Result of the model/prediction/output_SMOTE the result datatable/draw the ROC and PRC 
  #======================================================================================================
  ...
  cat(capture.output(print(summary(rf)),
                     file = paste0(getwd(),"/output_SMOTE/summaryrfGDSC_",drugg,".txt")))   ### print summary into a txt
  ...
  write.table(re,file = paste0(getwd(),"/output_SMOTE/PredictionGDSCtoCCLE_",drugg,".txt"),sep = '\t', col.names = TRUE,row.names = FALSE)
  ## write result table into a txt
  
  cat(capture.output(print(h2o.performance(model = rf, newdata = OnedrugTestH2o)),
                     file = paste0(getwd(),"/output_SMOTE/performance_GtoC_",drugg,".txt")))
  #======================================================================================================
  # ROC based on re
  #======================================================================================================
  ...
  write.table(deciP,file = paste0(getwd(),"/output_SMOTE/dicisionPATH",drugg,".txt"),sep = '\t', col.names = TRUE,row.names = FALSE)
...
write.table(plotall,file = paste0(getwd(),"/output_SMOTE/Allplot_CCLE.txt"),sep = '\t', col.names = TRUE,row.names = FALSE)
