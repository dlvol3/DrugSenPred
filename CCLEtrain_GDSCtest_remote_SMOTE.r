...
for (i in 1:length(levels(as.factor(IC50CCLE$drug)))){
  ...
  pdf(paste0(getwd(),"/output_SMOTE/",drugg,"_CCLE.pdf"))  # The pdf receiving the output
  ...
  write.table(ub,file = paste0(getwd(),"/output_SMOTE/CCLEGDSC_unbalanced",drugg,".txt"),
  ...
  cat(capture.output(print(summary(rf)),
                     file = paste0(getwd(),"/output_SMOTE/summaryrfCCLE_",drugg,".txt")))   ### print summary into a txt
