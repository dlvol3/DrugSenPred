...
for (i in 1:length(levels(as.factor(IC50CCLE$drug)))){
  ...
  pdf(paste0(getwd(),"/output_balanced/",drugg,"_GDSC.pdf"))  # The pdf receiving the output
  ...
  cat(capture.output(print(summary(rf)),
                     file = paste0(getwd(),"/output_balanced/summaryrfGDSC_",drugg,".txt")))   ### print summary into a txt
  ...
  write.table(re,file = paste0(getwd(),"/output_balanced/PredictionGDSCtoCCLE_",drugg,".txt"),sep = '\t', col.names = TRUE,row.names = FALSE)
